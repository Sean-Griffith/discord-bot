...
class Runescape(commands.Cog):
    ...
    async def traveling_merchant(self, ctx, option=None, *query):
        ...
        if(option == None):
            ...
            tms_data = self.rswiki_session.get(url).json()
            # Get tomorrows date
            tms_date += datetime.timedelta(days=1)
            
        elif(option == "3"):
            # Query rswiki to search for next appearance of query in Traveling Merchant Stock
            url = "https://api.weirdgloop.org/runescape/tms/search?name={}".format(query)
            ...
            if(isinstance(tms_data, list) and len(tms_data) > 0):
                tms_data = tms_data[0]['items']
                tms_date = datetime.datetime.strptime(self.rswiki_session.get(url).json()[0]['date'], "%d %B %Y")
        
        if(len(tms_data) > 3):
            await ctx.send(embed=RB.generated_tms_embed(tms_data, tms_date))
        else:
